...
@Component({
  selector: 'app-office-client-statistics',
  templateUrl: './office-client-statistics.component.html',
  styleUrls: ['./office-client-statistics.component.scss']
})
export class OfficeClientStatisticsComponent implements OnInit {
  flag = true;
  userid: any;
  chartDownload: any;
  ...
  ngOnInit(): void {
    this.userdetails.activeOfficerLink = 'Estadísticas por cliente'        
    if(this.flag == true) {
      this.render_chart();
    }  
  ...
  render_chart() {
    ...
    this.chartDownload = new CanvasJS.StockChart("chartDownloadRateContainer", {
      theme: "light3",
      title: {
    ...
    this.chartTotalAmount = new CanvasJS.StockChart("chartAmountContainer", {
      theme: "light3",
      title: {
    ...
    this.userdetails.getofficerclientdashboard(this.userid).subscribe(
      res => {
        const data:any = res;
